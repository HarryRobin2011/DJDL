...
//			console.log(currentField.add_num)
			var showboxHtml = '<div class="clickShowBox show" style="top: 0px;"><p><b>母鸡总数:</b><em>' +
	...
	function hatchChickenOrEgg(type, currId){
		$.ajax({
				...
				success: function(data) {
					...
					if(data.errcode == 10000) {
						if (type==1) {
							returnSucc_alert('成功收获 '+data.aninal_num+' 只母鸡到仓库中');
						}else{
		...
		$.ajax({
			...
			success: function(data) {
				if(data.errcode != 10000) {
					...
					var res = data.result;
					console.log(res);
					$('#ztNum').text(res.friendcount);
					$('#syNum').text(res.friendallcount);
...
//		return
		$.ajax({
			...
			success: function(data) {
				...
				if(data.errcode == 10000) {
					dasao = true;
					returnSucc_alert('收获成功，收益'+data.currency+'只母鸡')
//					setTimeout(function(){
//						window.location.reload();
//					},2000)
				} else {
