    ...
    $.ajax({
        ...
        success:function(data){
            if(data.errcode != 10000){
                ...
                $(res).each(function(){
                    if(this.active == 1){
                        $('.friend-list .show').append("<li class=\"number\"><i class=\"ico chick\"></i><div class=\"content\"> <em id=\"username\">"+this.account+"</em> <em id=\"regtime\">"+new Date(parseInt(this.reg_time) * 1000).toLocaleString().replace(/年|月/g, "-").replace(/日/g, " ")+"</em> </div> <button class=\"grayListBtn keyCleanDay\" ><span>已送母鸡</span></button> </li>");
                    }else{
                        $('.friend-list .show').append("<li class=\"number\"><i class=\"ico chick\"></i><div class=\"content\"> <em id=\"username\">"+this.account+"</em> <em id=\"regtime\">"+new Date(parseInt(this.reg_time) * 1000).toLocaleString().replace(/年|月/g, "-").replace(/日/g, " ")+"</em> </div> <button class=\"redListBtn goActive\"  data-id="+this.id+"><span>送母鸡</span></button> </li>");
                    }
                });
            }
        }
